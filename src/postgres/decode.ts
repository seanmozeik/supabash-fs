...
const decodeDocument = (value: unknown): BackendDocument => {
  ...
  if (!SHA256.test(bodyHash)) {
    throw corrupt('Snapshot document content hash is invalid.', path);
  }
  if (new TextEncoder().encode(body).byteLength !== byteSize) {
    throw corrupt('Snapshot document byte size does not match its UTF-8 body.', path);
  }
  if (body.includes('\0')) {
    throw new SupabashError('UNSUPPORTED_CONTENT', 'Postgres text contains NUL.', { path });
  }
  return { body, byteSize, contentHash: bodyHash, path };
};
